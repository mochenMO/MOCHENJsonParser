...
log::Logger* getJsonLogger()  // 获得JsonLogger的函数，写该函数是为了为多文件共享该JsonLogger做准备
{
...
JsonReader& JsonReader::operator[](const std::string _key)
{
	...
	return *this;   // 此时 m_string[m_index] = '}' 如果再进行 parse 会引发错误  ?????????????????????
}
...
JsonReader& JsonReader::operator[](int _index)
{
	...
	return *this;   // 此时 m_string[m_index] = ']' 如果进行再 parse 会引发错误  ??????????????????????????????????
}
